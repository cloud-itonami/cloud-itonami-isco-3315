(ns valuation.actor
  "ValuationActor — the ISCO-08 3315 independent valuation and loss
  assessment actor as a `langgraph.graph/state-graph` (ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one valuation or assessment
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-3314's
  statanalysis.actor.
  ...
  The unconditional invariant: the ValuationAdvisor can never
  directly issue a valuation or finalize an assessment the
  ValuationAssessmentGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [valuation.advisor :as advisor]
            [valuation.governor :as governor]
            [valuation.store :as store]))

(defn build-graph
  "Build a compiled ValuationActor graph. `store` implements
  `valuation.store/Store`. `advisor` implements
  `valuation.advisor/Advisor` (defaults to `mock-advisor`).
